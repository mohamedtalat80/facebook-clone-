...
class PostAPITestCase(APITestCase):
    ...
    def test_get_post_light(self):
        ...
        self.assertNotIn("content", response.data)  # النسخة الخفيفة ما فيهاش content

    def test_create_post(self):
        data = {"title": "New Post", "content": "This is a new post", "user": self.user.username}
...
class LikeAPITestCase(APITestCase):
    ...
    def test_create_like(self):
        data = {"user_like": self.user.username, "post_like": self.post.id, "liked": True}
...
class CommentAPITestCase(APITestCase):
    ...
    def test_add_comment(self):
        data = {"post_comment": self.post.id, "comment_content": "This is a test comment", "user_comment": self.user.username}
    ...
    def test_update_comment(self):
        comment_obj = comment.objects.create(user_comment=self.user, post_comment=self.post, comment_content="Test Comment")
        data = {"comment_content": "This is an updated comment"}
        response = self.client.post(f'/api/comments/{comment_obj.id}/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_add_reply(self):
        ...
        data = {"post_comment": self.post.id, "comment_content": "This is a test reply", "reply": parent_comment.id, "user_comment": self.user.username}
        response = self.client.post('/api/comments/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_update_reply(self):
        ...
        data = {"comment_content": "Updated Reply"}
...
class ProfileAPITestCase(APITestCase):
    ...
    def test_get_profile_light(self):
        ...
        self.assertNotIn("date_of_birth", response.data)  # النسخة الخفيفة ما فيهاش date_of_birth

    def test_create_profile(self):
        data = {"user": self.user.username, "bio": "New Bio", "date_of_birth": "2000-01-01", "status": "Single", "lives_in": "Cairo"}
